...
<?php if (FM_USE_HIGHLIGHTJS && isset($_GET['view'])): ?>
    ...
    <script>hljs.highlightAll(); let isHighlightingEnabled = true;</script>
<?php endif; ?>
<script>
    ...
    function checkbox_toggle() { let e = get_checkboxes(); e.push(this), change_checkboxes(e) }
    ...
    // Toast message
    function toast(txt) { let x = document.getElementById("snackbar");x.innerHTML=txt;x.className = "show";setTimeout(function(){ x.className = x.className.replace("show", ""); }, 3000); }
    // Save file
    function edit_save(e, t) {
        let n = "ace" == t ? editor.getSession().getValue() : document.getElementById("normal-editor").value;
        if (typeof n !== 'undefined' && n !== null) {
            if (true) {
                let data = {ajax: true, content: n, type: 'save', token: window.csrf};
                ...
                let c = document.createTextNode(n);
    ...
    // Advance search
    function fm_search() {
        let searchTxt = $("input#advanced-search").val(), searchWrapper = $("ul#search-wrapper"), path = $("#js-search-modal").attr("href"), _html = "", $loader = $("div.lds-facebook");
        if(!!searchTxt && searchTxt.length > 2 && path) {
            let data = {ajax: true, content: searchTxt, path:path, type: 'search', token: window.csrf };
            $.ajax({
                type: "POST",
                url: window.location,
                data: data,
...
<?php if (isset($_GET['edit']) && isset($_GET['env']) && FM_EDIT_FILE && !FM_READONLY):
    ...
    <script>
        ...
        function renderThemeMode() {
            let $modeEl = $("select#js-ace-mode"), $themeEl = $("select#js-ace-theme"), $fontSizeEl = $("select#js-ace-fontSize"), optionNode = function(type, arr){ let $Option = ""; $.each(arr, function(i, val) { $Option += "<option value='"+type+i+"'>" + val + "</option>"; }); return $Option; },
            ...
            if(_data && _data.aceTheme) { let lightTheme = optionNode("ace/theme/", _data.aceTheme.bright), darkTheme = optionNode("ace/theme/", _data.aceTheme.dark); $themeEl.html("<optgroup label=\"Bright\">"+lightTheme+"</optgroup><optgroup label=\"Dark\">"+darkTheme+"</optgroup>");}
