  ...
  for ($i = 0; sizeof($files) > $i; $i++) {
    if (!str_starts_with($files[$i], ".")) {
      ...
      if (@is_array(getimagesize($up.$path . "/" . $file_for_path))) {
        echo "<div data-filepath='" . $path . "/" . $file_for_path . "' class='group'><img class='image' alt='image " . $file_for_name . "' src='" . $up . $path . "/".  $file_for_path . "'><p>".$file_for_name."</p></div>";
      } else {
        if (is_dir($path . "/". $file_for_path) || str_contains("/", pathinfo($file_for_path, PATHINFO_EXTENSION))) {
          echo '<div data-filepath="' . $path . "/" . $file_for_path . '" class="group"><img alt="folder ' . $file_for_name . '" class="folder image" src="' . $root . '/assets/images/folder.png"><p>' . $file_for_name . '</p></div>';
        } else if (in_array(pathinfo($file_for_path, PATHINFO_EXTENSION), $text_ext)) {
          echo "<div data-filepath='" . $path . "/" . $file_for_path . "' class='group text'><h1 class='image'>" . pathinfo($file_for_path, PATHINFO_EXTENSION) . "</h1><p>" . $file_for_name . "</p></div>";
        } else {
          echo "<div data-filepath='" . $path . "/" . $file_for_path . "' class='group text'><h1 class='image' alt='image " . $file_for_name . "'>" . pathinfo($file_for_path, PATHINFO_EXTENSION) . "</h1><p>" . $file_for_name . "</p></div>";
        }
      }
    }
  }
